...
#pragma warning restore IDE0017 // 简化对象初始化
        ...
        /// <remarks>
        /// 此方法已经过时, 请使用<see cref="ReadKeyInSeconds(string, int, string, bool)"/>.
        /// 当控制台有输入法行时, 倒计时信息可能会出现重行的信息, 这是因为输入法行占据了一行的console缓冲区, 将导致计算行数不同于无输入法的状态；
        /// 由于目前无法实时识别控制台是否存在输入法行, 此bug目前无解.
        /// </remarks>
        [Obsolete]
        public static ConsoleKeyInfo ReadEnterInSeconds(string frontMessage, int timeout, string behindMessage = "", bool isShowTimeCountDown = true) {
            return ReadExpectedKeyInSeconds(ConsoleKey.Enter, frontMessage, timeout, behindMessage, isShowTimeCountDown);
        }
        ...
        /// <param name="isShowTimeCountDown">是否显示倒计时</param>
        /// <returns>读取到的按键信息</returns>
        public static ConsoleKeyInfo ReadExpectedKeyInSeconds(ConsoleKey expectedKey, string frontMessage, int timeout, string behindMessage = "", bool isShowTimeCountDown = true) {
